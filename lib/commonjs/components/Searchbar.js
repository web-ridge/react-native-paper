...
const Searchbar = ({
  ...
  const handleClearPress = e => {
    ...
    (_root$current = root.current) === null || _root$current === void 0 || _root$current.clear();
  ...
  const rippleColor = customRippleColor || (0, _color.default)(textColor).alpha(0.32).rgb().string();
  const traileringRippleColor = customTraileringRippleColor || (0, _color.default)(textColor).alpha(0.32).rgb().string();
  const font = isV3 ? {
    ...fonts.bodyLarge,
